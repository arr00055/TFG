...
public class PerfilHosteleroRestauranteVerMenu extends AppCompatActivity implements ProtocoloData {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_perfilhostmirestmismenus);
        this.setTitle(R.string.perfilhost_mirestaurante_infomenu_header1);
        Bundle bundle = getIntent().getExtras();
        Button BotonIntroducir = (Button) findViewById(R.id.perfil_host_mirestinfomenu_botonnummenu);
        ListView ListaViewMenus = (ListView) findViewById(R.id.perfil_host_mirestinfomenu_listviewMenus);

        if (bundle != null) {
            IDdelRestaurante = bundle.getString("IdRestHostaMisMenus");
        }
        ...
        try {
            IDMenusSalidaEstablece = BuscarEstablece.execute(IDdelRestaurante).get();
        ...
        BotonIntroducir.setOnClickListener(new View.OnClickListener() {
            public void onClick(View View) {
                EditText numermenu = (EditText) findViewById(R.id.perfil_host_mirestinfomenu_etinsertanummenu);
                String NumMenu = numermenu.getText().toString();
                if(NumMenu.isEmpty() == false) {
                    Toast.makeText(getApplicationContext(), "Accediendo al menú "+NumMenu, Toast.LENGTH_SHORT).show();
                    Intent HaciaPlatos = new Intent(PerfilHosteleroRestauranteVerMenu.this, PerfilHosteleroRestauranteVerMenuPlatos.class);
                    HaciaPlatos.putExtra("IDMenuDelRestauanteHost",NumMenu);
                    startActivity(HaciaPlatos);
                } else {
    ...
    public class BuscarEstablece extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute(){
            super.onPreExecute();
            pdia = new ProgressDialog(PerfilHosteleroRestauranteVerMenu.this);
